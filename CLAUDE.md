...
Several LangGraph engine modules (`contracts.py`, `provider.py`, `planning.py`, `github_approval.py`, `reentry.py`, `validate.py`) are explicitly documented as ports of specific functions from the legacy `plugins/agentic-sdlc/agentic_sdlc/__init__.py` (formerly `scripts/agentic_sdlc.py`, moved for pip/pipx packaging), with deliberate deviations (e.g. no module-level global state, since the engine must be reentrant across processes/CLI invocations).
